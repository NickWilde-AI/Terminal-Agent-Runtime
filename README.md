...
仓库内置 **智能终端演示域**（环境控制、多媒体、导航提示等）用于完整跑通闭环；同一套协议可扩展到手机、IoT、机器人、云运维、GUI / Coding Agent、Agent 平台等场景。
...
**演示域标杆任务：** 跨域复合目标（例如环境舒适度 + 媒体音量 + 导航提示保留），在约束、故障注入与用户改口下，仍然执行到**可回读的终态**，而不是只生成一段回复。
...
| **模型适配** | 默认阶跃 Step；`openai_compatible` 走标准 Tool Calling（`tools` + `tool_calls` + `role=tool`）；CI 用 `fake`；JSON 解析失败不降级 Fake |
| **设备模拟器** | 本地真值状态、延迟、故障注入、外部扰动，无需实机即可联调 |
| **3D 工作台** | React Three Fiber 程序化车辆；车窗/空调气流/媒体频谱/导航路线只绑定 Simulator Snapshot |
| **可观测** | Run 事件流（SSE）、只读回放、Trace 落盘；工作台实时展示 |
| **评测** | 固定用例；`agent`（状态反馈循环）与 `baseline`（一次展开）对照；断言独立于 Planner |
...
本仓库是**独立开源实现**，用来演示「有副作用系统」上的 Agent 执行语义。不含任何商业产品源码、内部接口、真实用户数据或设备日志；演示状态全部由本地模拟器生成。

| 本仓库包含 | 本仓库刻意不做 |
| --- | --- |
| 自研 Harness、FAST/AGENT、写后回读、取消隔离、评测 | 生产级 MCP Gateway |
| 标准 Tool Calling（`tools` / `tool_calls` / `role=tool`） | 向量数据库记忆（如 Milvus） |
| DevicePort + 本地 Simulator | 文档型 Agentic RAG |
| 受控结构化长期记忆（SQLite） | 真实设备 SDK / 生产网关 |
| `edge` placement 返回 `EDGE_UNAVAILABLE` | 端侧完整 Agent |
...
| --- | --- | --- | --- |
| **智能终端演示域**（仓库内置） | 环境、媒体、导航等设备真值 | 设定、开关、多域协同 | 回读满足目标与约束 |
...
| 2. 再试一句复杂目标 | 输入需要同时改环境、媒体、导航等多件事的长句 | 系统会分多步做，不会一句话就结束；右侧状态会一项项靠近目标 |
| 3. 故意制造异常 | 用「故障注入」选择：响应丢失、延迟生效、读取失败等 | 系统会先核对真实状态，再决定要不要继续；不应不管结果重复乱改 |
| 4. 中途改主意 | 任务还在跑时点「取消」，或输入「改成 25 度」 | 旧任务停住；新目标接管；不会继续执行已经作废的计划 |
| 5. 试试长期记忆 | 输入「记住我喜欢温度 24 度」，再开一个相关任务 | 记忆列表出现这条偏好；后续任务可把它当默认建议，但仍受安全策略约束 |
| 6. 跑一遍评测 | 点工作台里的评测，或用下面的命令 | 生成报告：多少用例通过、失败在哪；可对比「完整闭环」和「一次性展开」两种模式 |
...
覆盖面包括：简单指令、复杂多目标、故障、策略拒绝、取消与改目标、记忆规则等。  
两种评测模式：
