...
class SimulatorContractTest {
    ActionRecord write(DeviceSimulator s,String id,String cap,Map<String,Object> p){return s.applyWrite(id,id,cap,p,s.getEnvironmentId(),Instant.now().plusSeconds(5),s.snapshot().getDomainRevisions(),"run",1);}
    ...
    @Test void ackDoesNotChangeGeometrySource(){var s=new DeviceSimulator();s.injectFault(FaultType.ACK_NOT_APPLIED,"window.set_position",1);var a=write(s,"a","window.set_position",Map.of("window","front_left","position",100));assertEquals("NOT_APPLIED",a.status);assertEquals(0,s.snapshot().getState().get("window_front_left"));}
    @Test void lostResponseKeepsOriginalRecord(){var s=new DeviceSimulator();s.injectFault(FaultType.APPLIED_RESPONSE_LOST,"media.set_volume",1);assertThrows(SimulatorException.class,()->write(s,"a","media.set_volume",Map.of("value",2)));assertEquals("APPLIED",s.queryAction("a").status);long rev=s.snapshot().getRevision();write(s,"a","media.set_volume",Map.of("value",2));assertEquals(rev,s.snapshot().getRevision());}
    ...
    @Test void delayedActionRechecksExternalRevision(){var s=new DeviceSimulator();s.injectFault(FaultType.DELAY_APPLY,"media.set_volume",1);var a=write(s,"a","media.set_volume",Map.of("value",2));s.externalChange("media_volume",9);a.applyAfterMs=1;s.tick();assertEquals("NOT_APPLIED",a.status);assertEquals(9,s.snapshot().getState().get("media_volume"));assertEquals("EXTERNAL",s.snapshot().getOrigins().get("media_volume").get("origin"));}
